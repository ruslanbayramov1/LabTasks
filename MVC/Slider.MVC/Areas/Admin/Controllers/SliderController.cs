...
public class SliderController : Controller
{
    ...
    public async Task<IActionResult> Delete(int? id)
    {
        ...
        try
        {
            if (slider == null) throw new Exception($"The given id ({id}) is validating the data table");
            int res = await _sliderService.Delete(slider.Id);
            if (res != 0)
            {
        ...
        {
            return RedirectToAction("Index", "Error", new { area = "Admin", code = 400, message = ex.Message.ToString() });
        }
